...
class drone:
    ...
    def setstatus(self, newstatus):
        if newstatus in self.valid_statuses:
    ...
    def assignpackage(self, packageobj):
        ...
        if packageobj.weightinkg > self.maxloadinkg:
            print(f"drone {self.droneid}: package {packageobj.packageid} ({packageobj.weightinkg}kg) is too heavy (max {self.maxloadinkg}kg).")
            return False
